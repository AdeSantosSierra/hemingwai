...
def procesar_noticias():
    try:
        ...
        # Buscar la primera noticia sin 'puntuacion' en ninguna de las dos bases
        for doc in old_collection.find({}):
            ...
            if ('puntuacion' not in doc or doc['puntuacion'] in (None, '')):
                ...
                if not doc_nueva or ('puntuacion' not in doc_nueva or doc_nueva['puntuacion'] in (None, '')):
                    # Copiar el documento completo a la nueva
                    new_collection.replace_one({'_id': _id}, doc, upsert=True)
                    # Analizar la noticia usando la base nueva
                    ...
                    new_collection.update_one({"_id": _id}, {"$set": safe_fields}, upsert=True)
                    print(
                    ...
                    break  # Solo procesa una noticia por ejecución
